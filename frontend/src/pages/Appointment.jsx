...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      // Setting start time of the date with index
      ...
      while (currentDate < endTime) {
        ...
        const isSlotAvailable = docInfo.slots_booked[slotDate] && docInfo.slots_booked[slotDate].includes(formatedTime)? false:true
        if(isSlotAvailable){
          // add slot to array
          timeSlots.push({
  ...
  const bookAppointment = async () => {
    if (!slotTime || !docSlots[slotIndex]?.length) {
      toast.warn('Select a date and time slot')
      return
    }
    if (!token) {
    ...
    <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4'>
          {
            docSlots.length && docSlots.map((item, index) => {
              return (
                <div onClick={() => setSlotIndex(index)} key={index} className={`text-center py-6 min-w-16 rounded-full cuwsor-pointer ${slotIndex === index ? 'bg-primary text-white' : 'border border-gray-200'}`}>
        ...
        <button
          ...
          className={`text-white text-sm font-light px-14 py-3 rounded-full my-6 ${slotTime && docSlots[slotIndex]?.length ? 'bg-primary' : 'bg-gray-400 cursor-not-allowed'}`}
        >
          Book an appointment
        </button>
      </div>
      {/* --- related doctors components --- */}
