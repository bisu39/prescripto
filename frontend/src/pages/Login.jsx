    ...
    <div>
      <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
        <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-85 sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
          <p className='text-2xl font-semibold'>{state === 'Sing Up' ? 'Create Account' : 'Log in'}</p>
          ...
          <button type='submit' className='bg-primary text-white w-full py-2 rounded-md text-base'>{state === 'Sing Up' ? 'Create Account' : 'Login'}</button>
          {
            state === "Sing Up" ?
              ...
              <p>Create an new account ? <span className='text-primary underline cursor-pointer' onClick={() => setState('Sing Up')}>click here</span> </p>
          }
        </div>
      </form>
